...
### Week 7 (2026-06-08)
- [x] `AchievementChecker` silent level-up fixed — `GameManager.ApplyLevelReward` made public; level-up from achievement XP now applies rewards — `GameManager.cs`
- [x] Daily-bonus level-up rewards applied — `DailyChallenge.TryClaimDailyBonus` returns `out int newLevel`; `TombakanOnboarding.ShowDailyBonus` calls `ApplyLevelReward` — `DailyChallenge.cs`, `TombakanOnboarding.cs`, `GameManager.cs`
- [x] `ColourBlindSettings.ShapeForColor` combined palette dict + hue-proximity — no more `"?"` for any species colour — `ColourBlindSettings.cs`
- [x] `ShowSad()` shows actual deduction — `"Miss!"` when score is 0, exact amount otherwise — `GameManager.cs`
